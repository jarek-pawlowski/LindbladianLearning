...
class TrajectoryDataset(Dataset):
    ...
    def __init__(self, root_dir: str, label_transform: str = "log"):
        """
        :param:
    ...
    def __getitem__(self, idx):
        ...
        features_np = traj_to_real_features(rhos, times, timestep)

        if self._feature_dim is None:
            self._feature_dim = features_np.shape[0]    # get the dimensionality of the features from the first trajectory
            self._n_steps_plus_1 = rhos.shape[0]        # get the number of timesteps from the first trajectory

        labels_np = 1e12*np.array([float(data[k]) for k in TAU_KEYS], dtype=np.float64)  # parameters as a float64 array
